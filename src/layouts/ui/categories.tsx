...
const INITIAL_FORM: FormData = { name: "", type: "EXPENSE", icon: "📁", color: "#8B5CF6", isDefault: false };
  ...
  return (
    <Card variant="elevated" className="transition-all duration-300 hover:shadow-xl group">
      <CardContent className="pt-4 sm:pt-6">
        ...
        <h3 className="mb-1.5 text-base sm:text-lg font-bold truncate text-primary-900 sm:mb-2" title={category.name}>
          {category.name}
        </h3>
    ...
    <Card className="col-span-full">
      <CardContent className="pt-4 sm:pt-6">
        <div className="py-10 text-center sm:py-16">
          ...
          <h3 className="mb-1.5 text-lg font-bold sm:mb-2 sm:text-xl text-primary-900">{message.title}</h3>
          <p className="max-w-xs mx-auto mb-5 text-sm sm:max-w-md sm:mb-6 text-primary-600">{message.description}</p>
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title={title} size="md">
      <div className="space-y-3 sm:space-y-4">
        <div className="p-2.5 sm:p-3 border rounded-lg bg-primary-50 border-primary-200">
        ...
        <div className="flex items-center gap-2.5 p-2.5 sm:gap-3 sm:p-3 rounded-lg bg-neutral">
          ...
          />
          <label htmlFor="isDefaultCategory" className="flex-1 text-xs font-medium sm:text-sm text-primary-900">
            {t("modal.setDefault")}
            <span className="block mt-0.5 text-xs text-primary-600">{t("modal.defaultHint")}</span>
          </label>
        </div>
        ...
        <div className="p-3 border rounded-lg sm:p-4 bg-linear-to-br from-primary-50 to-neutral border-primary-200">
          ...
          <div className="flex items-center gap-2.5 p-2.5 bg-white rounded-lg shadow-sm sm:gap-3 sm:p-3">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-sm font-medium truncate text-primary-900">{formData.name || t("modal.categoryNameLabel")}</p>
...
export const Categories: React.FC = () => {
  ...
  const handleEdit = React.useCallback((category: Category): void => {
    ...
    setFormData({ name: category.name, type: category.type, icon: category.icon || "📁", color: category.color || "#8B5CF6", isDefault: category.isDefault || false });
    setIsModalOpen(true);
  }, []);
  ...
  return (
    <div className="space-y-3 sm:space-y-5 lg:space-y-6">
        ...
        <Card>
          <CardContent className="pt-4 sm:pt-6">
            <div className="grid grid-cols-2 gap-3 sm:gap-4 md:grid-cols-4">
              {[
                { value: stats.total, label: t("stats.total"), color: "text-primary-900" },
                { value: stats.income, label: t("income"), color: "text-green-600" },
                { value: stats.expense, label: t("expense"), color: "text-red-600" },
                { value: stats.custom, label: t("stats.custom"), color: "text-primary-900" },
              ].map(({ value, label, color }) => (
                <div key={label} className="text-center">
                  <p className={`text-xl font-bold sm:text-2xl tabular-nums ${color}`}>{value}</p>
                  <p className="mt-0.5 text-xs sm:mt-1 sm:text-sm text-primary-600">{label}</p>
                </div>
              ))}
            </div>
          </CardContent>
      ...
      {(filterType === "ALL" || filterType === "INCOME") && (
        <div>
          <h2 className="mb-3 text-base font-bold sm:mb-4 sm:text-xl text-primary-900">💰 {t("incomeCategories", { count: incomeCategories.length })}</h2>
      ...
      {(filterType === "ALL" || filterType === "EXPENSE") && (
        <div>
          <h2 className="mb-3 text-base font-bold sm:mb-4 sm:text-xl text-primary-900">💳 {t("expenseCategories", { count: expenseCategories.length })}</h2>
