...
export const POST = withApi("accounts.create", async (req: NextRequest) => {
  const user = await requireAuth();
  ...
  if (userAccountCount >= maxAccounts) {
    logger.warn("accounts.limit_reached", { current: userAccountCount, max: maxAccounts });
    return errorResponse("Maximum number of accounts reached", 400);
  }
