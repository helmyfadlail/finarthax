...
import { DETECTION_DEFAULTS, detectRecurringPatterns, diffInDays, getUserPreferences, monthlyEquivalent, prisma, readNumberPreference, requireAuth, resolveStatus, withMaintenanceGuard } from "@/lib";
  ...
  return withMaintenanceGuard(req, async () => {
    try {
      ...
      const validation = recurringFilterSchema.safeParse({
        ...
        historyDays: Number(searchParams.get("historyDays") ?? DETECTION_DEFAULTS.historyDays),
        minOccurrences: Number(searchParams.get("minOccurrences") ?? DETECTION_DEFAULTS.minOccurrences),
      });
